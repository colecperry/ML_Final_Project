...
# 3-5) Turn text into TF-IDF vectors (default parameters), scale the metadata (publisher_id and date_ordinal),
#      combine text features and metadata into one array for each split handled in the ColumnTransformer below
...
# 9) Repeat the exact same pipeline, but load the BALANCED splits instead
...
# 10) Separate text / metadata from each balanced DataFrame
X_train_b_df = train_b
X_val_b_df   = val_b
X_test_b_df  = test_b

# 14) 5-Fold Cross-Validation on balanced training data before hyperparameter tuning
run_cv(pipeline_default, X_train_b_df, y_train_b, 'Balanced Training Set (pre-tuning)')

# 15) Train & eval on balanced data before hyperparameter tuning
pipeline_default.fit(X_train_b_df, y_train_b)
...
# 16) Hyperparameter tuning for TF-IDF on balanced data using GridSearchCV
param_grid = {
    'pre__text__max_features': [11000, 12000, 13000],
    'pre__text__min_df':       [1, 2, 3],
    'pre__text__max_df':       [0.4, 0.5, 0.6],
    'pre__text__ngram_range':  [(1,2), (1,3), (2,3)]
}
...
# 17) Nested CV for thresholding and hyperparameter selection
...
# 18 & 19) Final Test Results After Threshold Tuning
